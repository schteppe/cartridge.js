...
this.delayInputPointer=this.delayInputPointer-this.delayBufferSamples.length}};SingleDelay.prototype.setDelayVolume=function(delayVolume){this.delayVolume=delayVolume};SingleDelay.prototype.process=function(samples){var outputSamples=new Float64Array(samples.length);for(var i=0;i<samples.length;i++){this.delayBufferSamples[this.delayInputPointer]=samples[i];var delaySample=this.delayBufferSamples[this.delayOutputPointer];outputSamples[i]=delaySample*this.delayVolume;this.delayInputPointer++;if(this.delayInputPointer>=this.delayBufferSamples.length-1){this.delayInputPointer=0}this.delayOutputPointer++;if(this.delayOutputPointer>=this.delayBufferSamples.length-1){this.delayOutputPointer=0}}return outputSamples};function Reverb(maxDelayInSamplesSize,delayInSamples,masterVolume,mixVolume,delayVolume,dampFrequency){this.delayInSamples=delayInSamples;this.masterVolume=masterVolume;this.mixVolume=mixVolume;this.delayVolume=delayVolume;this.dampFrequency=dampFrequency;this.NR_OF_MULTIDELAYS=6;this.NR_OF_SINGLEDELAYS=6;this.LOWPASSL=new IIRFilter2(DSP.LOWPASS,dampFrequency,0,44100);this.LOWPASSR=new IIRFilter2(DSP.LOWPASS,dampFrequency,0,44100);this.singleDelays=[];var i,delayMultiply;for(i=0;i<this.NR_OF_SINGLEDELAYS;i++){delayMultiply=1+i/7;this.singleDelays[i]=new SingleDelay(maxDelayInSamplesSize,Math.round(this.delayInSamples*delayMultiply),this.delayVolume)}this.multiDelays=[];for(i=0;i<this.NR_OF_MULTIDELAYS;i++){delayMultiply=1+i/10;this.multiDelays[i]=new MultiDelay(maxDelayInSamplesSize,Math.round(this.delayInSamples*delayMultiply),this.masterVolume,this.delayVolume)}}Reverb.prototype.setDelayInSamples=function(delayInSamples){this.delayInSamples=delayInSamples;var i,delayMultiply;for(i=0;i<this.NR_OF_SINGLEDELAYS;i++){delayMultiply=1+i/7;this.singleDelays[i].setDelayInSamples(Math.round(this.delayInSamples*delayMultiply))}for(i=0;i<this.NR_OF_MULTIDELAYS;i++){delayMultiply=1+i/10;this.multiDelays[i].setDelayInSamples(Math.round(this.delayInSamples*delayMultiply))}};Reverb.prototype.setMasterVolume=function(masterVolume){this.masterVolume=masterVolume};Reverb.prototype.setMixVolume=function(mixVolume){this.mixVolume=mixVolume};Reverb.prototype.setDelayVolume=function(delayVolume){this.delayVolume=delayVolume;var i;for(i=0;i<this.NR_OF_SINGLEDELAYS;i++){this.singleDelays[i].setDelayVolume(this.delayVolume)}for(i=0;i<this.NR_OF_MULTIDELAYS;i++){this.multiDelays[i].setDelayVolume(this.delayVolume)}};Reverb.prototype.setDampFrequency=function(dampFrequency){this.dampFrequency=dampFrequency;this.LOWPASSL.set(dampFrequency,0);this.LOWPASSR.set(dampFrequency,0)};Reverb.prototype.process=function(interleavedSamples){var outputSamples=new Float64Array(interleavedSamples.length);var leftRightMix=DSP.deinterleave(interleavedSamples);this.LOWPASSL.process(leftRightMix[DSP.LEFT]);this.LOWPASSR.process(leftRightMix[DSP.RIGHT]);var filteredSamples=DSP.interleave(leftRightMix[DSP.LEFT],leftRightMix[DSP.RIGHT]);var i;for(i=0;i<this.NR_OF_MULTIDELAYS;i++){outputSamples=DSP.mixSampleBuffers(outputSamples,this.multiDelays[i].process(filteredSamples),2%i===0,this.NR_OF_MULTIDELAYS)}var singleDelaySamples=new Float64Array(outputSamples.length);for(i=0;i<this.NR_OF_SINGLEDELAYS;i++){singleDelaySamples=DSP.mixSampleBuffers(singleDelaySamples,this.singleDelays[i].process(outputSamples),2%i===0,1)}for(i=0;i<singleDelaySamples.length;i++){singleDelaySamples[i]*=this.mixVolume}outputSamples=DSP.mixSampleBuffers(singleDelaySamples,interleavedSamples,0,1);for(i=0;i<outputSamples.length;i++){outputSamples[i]*=this.masterVolume}return outputSamples};if(module&&typeof module.exports!=="undefined"){module.exports={DSP:DSP,DFT:DFT,FFT:FFT,RFFT:RFFT,Sampler:Sampler,Oscillator:Oscillator,ADSR:ADSR,IIRFilter:IIRFilter,IIRFilter2:IIRFilter2,WindowFunction:WindowFunction,sinh:sinh,Biquad:Biquad,GraphicalEq:GraphicalEq,MultiDelay:MultiDelay,SingleDelay:SingleDelay,Reverb:Reverb}}},{}],2:[function(require,module,exports){exports.defaultPalette=function(){return[0,1911635,8267091,34641,11227702,6248271,12764103,16773608,16711757,16753408,16773156,59222,2731519,8615580,16742312,16764074]};exports.int2hex=function(int){var hex=int.toString(16);while(hex.length<6){hex="0"+hex}hex="#"+hex;return hex}},{}],3:[function(require,module,exports){var utils=require("./utils");var fontImageAsCanvas;var coloredFontCanvases=[];var fontX=4;var fontY=5;var paletteHex=[];var chars='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,^?()[]:/\\="a+-!{}<>;_|&*~%';exports.init=function(fontImage,paletteHex){for(var i=0;i<paletteHex.length;i++){var coloredFontCanvas=document.createElement("canvas");coloredFontCanvases.push(coloredFontCanvas)}fontImageAsCanvas=document.createElement("canvas");fontImageAsCanvas.width=fontImage.width;fontImageAsCanvas.height=fontImage.height;fontImageAsCanvas.getContext("2d").drawImage(fontImage,0,0,fontImage.width,fontImage.height);exports.changePalette(paletteHex)};exports.changePalette=function(paletteHex){if(!fontImageAsCanvas)return;var fontImageData=fontImageAsCanvas.getContext("2d").getImageData(0,0,fontImageAsCanvas.width,fontImageAsCanvas.height);for(var i=0;i<paletteHex.length;i++){var data=fontImageData.data;for(var j=0;j<data.length/4;j++){if(!(data[4*j+0]===0&&data[4*j+1]===0&&data[4*j+2]===0&&data[4*j+3]===0)){var rgb=utils.hexToRgb(paletteHex[i]);data[4*j+0]=rgb[0];data[4*j+1]=rgb[1];data[4*j+2]=rgb[2]}}coloredFontCanvases[i].getContext("2d").putImageData(fontImageData,0,0)}};exports.draw=function(ctx,text,x,y,col){for(var i=0;i<text.length;i++){var index=chars.indexOf(text[i]);if(index!==-1){ctx.drawImage(coloredFontCanvases[col],index*fontX,0,fontX,fontY,x+fontX*i,y,fontX,fontY)}}};exports.load=function(callback){var im=new Image;im.onload=function(){callback(im)};im.src="data:image/png;base64,"}},{"./utils":10}],4:[function(require,module,exports){exports.hello=function(){console.log(["CARTRIDGE.JS","JavaScript retro game engine.","","For help, run help()."].join("\n"))};exports.print=function(){console.log(["fullscreen()      Enable full screen.","help()            Print this message.",'load("name")      Load sprites and map from localStorage.','save("name")      Save sprites and map to localStorage.','save("name.json") Download the contents to a JSON file.'].join("\n"))}},{}],5:[function(require,module,exports){var help=require("./help");var input=require("./input");var utils=require("./utils");var font=require("./font");var math=require("./math");var colors=require("./colors");var sfx=require("./sfx");var pixelops=require("./pixelops");var cellsizeX=8;var cellsizeY=8;var screensizeX=128;var screensizeY=128;var mapSizeX=128;var mapSizeY=32;var spriteSheetSizeX=16;var spriteSheetSizeY=16;var maxSprites=spriteSheetSizeX*spriteSheetSizeY;var container;var canvases=[];var canvasListeners;var bodyListeners;var mapData=utils.zeros(mapSizeX*mapSizeY);var mapDataDirty=utils.zeros(mapSizeX*mapSizeY);var mapDirty=true;var mapCacheCanvas;var mapCacheContext;var spriteSheetCanvas;var spriteSheetContext;var spriteFlags=utils.zeros(maxSprites);var spriteSheetPixels;var ctx;var _time=0;var camX=0;var camY=0;var palette;var paletteHex;var defaultColor=0;var transparentColors=utils.zeros(16).map(function(){return false});transparentColors[0]=true;var loaded=false;var _alpha=0;var code="";exports.cartridge=function(options){screensizeX=options.width!==undefined?options.width:128;screensizeY=options.height!==undefined?options.height:128;cellsizeX=options.cellwidth!==undefined?options.cellwidth:8;cellsizeY=options.cellheight!==undefined?options.cellheight:8;var autoFit=options.autoFit!==undefined?options.autoFit:true;var numCanvases=options.layers!==undefined?options.layers:1;container=options.containerId?document.getElementById(options.containerId):null;container.innerHTML="";for(var i=0;i<numCanvases;i++){container.innerHTML+='<canvas class="cartridgeCanvas" id="cartridgeCanvas'+i+'" width="'+screensizeX+'" height="'+screensizeY+'"'+(i===0?" moz-opaque":"")+"></canvas>"}for(var i=0;i<numCanvases;i++){var c=document.getElementById("cartridgeCanvas"+i);c.oncontextmenu=function(){return false};canvases.push(c);if(i!==0){c.style.pointerEvents="none"}c.style.position="absolute";utils.disableImageSmoothing(c.getContext("2d"))}setCellSize(cellsizeX,cellsizeY);setPalette(options.palette||colors.defaultPalette());var style=document.createElement("style");style.innerHTML=[".cartridgeCanvas {","image-rendering: -moz-crisp-edges;","image-rendering: -webkit-crisp-edges;","image-rendering: pixelated;","}"].join("\n");document.getElementsByTagName("head")[0].appendChild(style);canvas(0);input.init(canvases);pixelops.init(canvases[0]);fit();if(autoFit){document.body.onresize=document.body.mozfullscreenchange=function(){fit()}}var currentTime=0;var t0=0;var t1=0;var dt0=Math.floor(1/30*1e3);var dt1=Math.floor(1/60*1e3);var accumulator0=0;var accumulator1=0;function render(newTime){if(currentTime){var frameTime=newTime-currentTime;if(frameTime>250)frameTime=250;accumulator0+=frameTime;while(accumulator0>=dt0){_time=t0;t0+=dt0;accumulator0-=dt0;_alpha=accumulator0/dt0;if(loaded&&typeof _update!=="undefined"){try{_update()}catch(err){console.error(err)}}}accumulator1+=frameTime;while(accumulator1>=dt1){_time=t1;t1+=dt1;accumulator1-=dt1;_alpha=accumulator1/dt1;if(loaded&&typeof _update60!=="undefined"){try{_update60()}catch(err){console.error(err)}}}}_time=newTime;if(loaded&&typeof _draw!=="undefined"){try{_draw()}catch(err){console.error(err)}}pixelops.flush();currentTime=newTime;input.update();requestAnimationFrame(render)}requestAnimationFrame(render);font.load(function(image){font.init(image,paletteHex);if(code){run()}if(typeof _load!=="undefined"){try{_load(postLoad)}catch(err){console.error(err);postLoad(err)}}else{postLoad()}})};function postLoad(err){loaded=true;if(typeof _init!=="undefined"){try{_init()}catch(err){console.error(err)}}}function setCellSize(w,h){w=w|0;h=h|0;cellsizeX=w;cellsizeY=h;spriteSheetPixels=utils.zeros(spriteSheetSizeX*spriteSheetSizeY*cellsizeX*cellsizeY);spriteSheetCanvas=utils.createCanvas(spriteSheetSizeX*cellsizeX,spriteSheetSizeY*cellsizeY);spriteSheetContext=spriteSheetCanvas.getContext("2d");mapCacheCanvas=utils.createCanvas(mapSizeX*cellsizeX,mapSizeY*cellsizeY);mapCacheContext=mapCacheCanvas.getContext("2d")}function redrawSpriteSheet(){var w=spriteSheetSizeX*cellsizeX;var h=spriteSheetSizeY*cellsizeY;for(var i=0;i<w;i++){for(var j=0;j<h;j++){redrawSpriteSheetPixel(i,j)}}}function redrawSpriteSheetPixel(x,y){var w=spriteSheetSizeX*cellsizeX;var h=spriteSheetSizeY*cellsizeY;var col=spriteSheetPixels[y*w+x];if(transparentColors[col]){spriteSheetContext.clearRect(x,y,1,1)}else{spriteSheetContext.fillStyle=paletteHex[col%palette.length];spriteSheetContext.fillRect(x,y,1,1)}}function setPalette(p){palette=p.slice(0);paletteHex=palette.map(colors.int2hex);mapDirty=true;redrawSpriteSheet();font.changePalette(paletteHex)}exports.palset=function(n,hexColor){var newPalette=palette.slice(0);newPalette[n]=hexColor;setPalette(newPalette)};exports.palget=function(n){return palette[n]};function resizeCanvases(){for(var i=0;i<canvases.length;i++){canvases[i].width=screensizeX;canvases[i].height=screensizeY}fit();pixelops.resize(canvases[0])}exports.alpha=function(){return _alpha};exports.width=function(newWidth){if(newWidth!==undefined){screensizeX=newWidth|0;resizeCanvases()}return screensizeX};exports.height=function(newHeight){if(newHeight!==undefined){screensizeY=newHeight|0;resizeCanvases()}return screensizeY};exports.cellwidth=function(newCellWidth){if(newCellWidth!==undefined){setCellSize(newCellWidth,cellsizeY)}else{return cellsizeX}};exports.cellheight=function(newCellHeight){if(newCellHeight!==undefined){setCellSize(cellsizeX,newCellHeight)}else{return cellsizeY}};exports.cls=function(){pixelops.beforeChange();ctx.clearRect(-camX,-camY,screensizeX,screensizeY)};exports.time=function(){return _time/1e3};exports.color=function(col){defaultColor=col};exports.palt=function(col,t){transparentColors[col]=t};exports.rectfill=function rectfill(x0,y0,x1,y1,col){pixelops.beforeChange();x0=x0|0;y0=y0|0;x1=x1|0;y1=y1|0;col=col!==undefined?col:defaultColor;var w=x1-x0+1;var h=y1-y0+1;ctx.fillStyle=paletteHex[col];ctx.fillRect(x0,y0,w,h)};exports.rect=function rect(x0,y0,x1,y1,col){pixelops.beforeChange();x0=x0|0;y0=y0|0;x1=x1|0;y1=y1|0;col=col!==undefined?col:defaultColor;var w=x1-x0;var h=y1-y0;ctx.fillStyle=paletteHex[col];ctx.fillRect(x0,y0,w,1);ctx.fillRect(x0,y0,1,h);ctx.fillRect(x1,y0,1,h+1);ctx.fillRect(x0,y1,w+1,1)};exports.clip=function(x,y,w,h){x=x|0;y=y|0;w=w|0;h=h|0;ctx.rect(x,y,w,h);ctx.clip()};exports.canvas=function canvas(n){ctx=canvases[n].getContext("2d")};exports.camera=function camera(x,y){x=x|0;y=y|0;if(camX===x&&camY===y)return;ctx.translate(x-camX,y-camY);camX=x;camY=y};exports.map=function map(cel_x,cel_y,sx,sy,cel_w,cel_h,layer){pixelops.beforeChange();layer=layer===undefined?0:layer;cel_x=cel_x|0;cel_y=cel_y|0;sx=sx|0;sy=sy|0;cel_w=cel_w|0;cel_h=cel_h|0;var i,j;if(layer===0){if(mapDirty){for(i=0;i<mapSizeX;i++){for(j=0;j<mapSizeY;j++){updateMapCacheCanvas(i,j)}}mapDirty=false}for(i=0;i<mapSizeX;i++){for(j=0;j<mapSizeY;j++){if(mapDataDirty[j*mapSizeX+i]){updateMapCacheCanvas(i,j);mapDataDirty[j*mapSizeX+i]=0}}}var _sx=cel_x*cellsizeX;var _sy=cel_y*cellsizeY;var _x=sx;var _y=sy;var _swidth=cel_w*cellsizeX;var _sheight=cel_h*cellsizeY;var _width=_swidth;var _height=_sheight;ctx.drawImage(mapCacheCanvas,_sx,_sy,_swidth,_sheight,_x,_y,_width,_height)}else{for(i=0;i<cel_w;i++){for(j=0;j<cel_h;j++){var spriteNumber=mget(i,j);var flags=fget(spriteNumber);if((layer&flags)===layer){spr(spriteNumber,sx+i*cellsizeX,sy+j*cellsizeY)}}}}};function ssx(n){return n%16}function ssy(n){return Math.floor(n/16)%(16*16)}exports.spr=function spr(n,x,y,w,h,flip_x,flip_y){pixelops.beforeChange();w=w!==undefined?w:1;h=h!==undefined?h:1;flip_x=flip_x!==undefined?flip_x:false;flip_y=flip_y!==undefined?flip_y:false;x=x|0;y=y|0;w=w|0;h=h|0;var sizex=cellsizeX*w;var sizey=cellsizeY*h;ctx.save();ctx.translate(x+(flip_x?sizex:0),y+(flip_y?sizey:0));ctx.scale(flip_x?-1:1,flip_y?-1:1);ctx.drawImage(spriteSheetCanvas,ssx(n)*cellsizeX,ssy(n)*cellsizeY,sizex,sizey,0,0,sizex,sizey);ctx.restore()};exports.fget=function(n){return spriteFlags[n]};exports.fset=function(n,flags){spriteFlags[n]=flags};exports.pget=function(){var data=new Uint8Array(3);return function(x,y){x=x|0;y=y|0;pixelops.pget(x,y,data);var col=utils.rgbToDec(data[0],data[1],data[2]);return palette.indexOf(col)}}();exports.pset=function(x,y,col){x=x|0;y=y|0;col=col|0;var dec=palette[col];var r=utils.decToR(dec);var g=utils.decToG(dec);var b=utils.decToB(dec);pixelops.pset(x,y,r,g,b)};exports.sget=function(x,y){x=x|0;y=y|0;var w=spriteSheetSizeX*cellsizeX;return spriteSheetPixels[y*w+x]};exports.sset=function(x,y,col){x=x|0;y=y|0;col=col!==undefined?col:defaultColor;var w=spriteSheetSizeX*cellsizeX;spriteSheetPixels[y*w+x]=col;redrawSpriteSheetPixel(x,y);mapDirty=true};exports.fullscreen=function fullscreen(){utils.fullscreen(container)};exports.print=function(text,x,y,col){pixelops.beforeChange();if(Array.isArray(text)){for(var i=0;i<text.length;i++){exports.print(text[i],x,y+8*i,col)}return}x=x!==undefined?x:0;y=y!==undefined?y:0;col=col!==undefined?col:defaultColor;x=x|0;y=y|0;font.draw(ctx,text.toUpperCase(),x,y,col)};exports.fit=function fit(){var i=canvases.length;while(i--){utils.scaleToFit(canvases[i],container,true)}};exports.mget=function mget(x,y){x=x|0;y=y|0;return mapData[y*mapSizeX+x]};exports.mset=function mset(x,y,i){if(mget(x,y)===i)return;x=x|0;y=y|0;mapData[y*mapSizeX+x]=i;mapDataDirty[y*mapSizeX+x]=1};exports.save=function(key){key=key||"save";var data=toJSON();var idx=key.indexOf(".json");if(idx!==-1){download(key.substr(0,idx))}else{localStorage.setItem(key,JSON.stringify(data))}};exports.load=function(key){key=key||"save";if(key.indexOf(".json")!==-1){loadJsonFromUrl(key,function(err,json){if(json){loadJSON(json);run()}})}else{try{var data=JSON.parse(localStorage.getItem(key));loadJSON(data);return true}catch(err){return false}}};function loadJsonFromUrl(url,callback){var xhr=new XMLHttpRequest;xhr.onreadystatechange=function(){if(xhr.readyState===XMLHttpRequest.DONE){if(xhr.status===200){callback(null,JSON.parse(xhr.responseText))}else{callback(xhr)}}};xhr.open("GET",url,true);xhr.send()}exports.codeset=function(codeString){code=codeString};exports.codeget=function(){return code};exports.run=function run(){eval.call(null,codeget())};function download(key){key=key||"export";var data=toJSON();var url=URL.createObjectURL(new Blob([JSON.stringify(data)]));var a=document.createElement("a");a.href=url;a.download=key+".json";document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url)}function toJSON(){var data={version:4,width:screensizeX,height:screensizeY,cellwidth:cellwidth(),cellheight:cellheight(),map:[],sprites:[],flags:[],palette:palette.slice(0),sfx:[],code:codeget()};for(var i=0;i<spriteFlags.length;i++){data.flags[i]=fget(i)}for(var i=0;i<spriteSheetSizeX*cellwidth();i++){for(var j=0;j<spriteSheetSizeY*cellheight();j++){data.sprites[j*spriteSheetSizeX*cellwidth()+i]=sget(i,j)}}for(var i=0;i<mapSizeX;i++){for(var j=0;j<mapSizeY;j++){data.map[j*mapSizeX+i]=mget(i,j)}}for(var n=0;n<64;n++){data.sfx[n]={speed:asget(n),volumes:[],pitches:[],waves:[]};for(var offset=0;offset<32;offset++){data.sfx[n].volumes.push(avget(n,offset));data.sfx[n].pitches.push(afget(n,offset));data.sfx[n].waves.push(awget(n,offset))}}return data}function loadJSON(data){codeset(data.code||"");if(data.width!==undefined){width(data.width)}if(data.height!==undefined){height(data.height)}if(data.cellwidth!==undefined){cellwidth(data.cellwidth)}if(data.cellheight!==undefined){cellheight(data.cellheight)}for(var i=0;i<spriteFlags.length;i++){fset(i,data.flags[i])}setPalette(data.palette);for(var i=0;i<spriteSheetSizeX*cellwidth();i++){for(var j=0;j<spriteSheetSizeY*cellheight();j++){sset(i,j,data.sprites[j*spriteSheetSizeX*cellwidth()+i])}}for(var i=0;i<mapSizeX;i++){for(var j=0;j<mapSizeY;j++){mset(i,j,data.map[j*mapSizeX+i])}}for(var n=0;n<data.sfx.length;n++){asset(n,data.sfx[n].speed);for(var offset=0;offset<data.sfx[n].volumes.length;offset++){avset(n,offset,data.sfx[n].volumes[offset]);afset(n,offset,data.sfx[n].pitches[offset]);awset(n,offset,data.sfx[n].waves[offset])}}redrawSpriteSheet()}exports.loadjson=loadJSON;function updateMapCacheCanvas(x,y){var n=mget(x,y);mapCacheContext.clearRect(x*cellsizeX,y*cellsizeY,cellsizeX,cellsizeY);mapCacheContext.drawImage(spriteSheetCanvas,ssx(n)*cellsizeX,ssy(n)*cellsizeY,cellsizeX,cellsizeY,cellsizeX*x,cellsizeY*y,cellsizeX,cellsizeY)}exports.help=function(){help.print()};exports.mousex=function(){return Math.floor(input.mousexNormalized()*screensizeX)};exports.mousey=function(){return Math.floor(input.mouseyNormalized()*screensizeY)};utils.makeGlobal(math);utils.makeGlobal(sfx);utils.makeGlobal(exports);utils.makeGlobal(input.global);help.hello();help.print()},{"./colors":2,"./font":3,"./help":4,"./input":6,"./math":7,"./pixelops":8,"./sfx":9,"./utils":10}],6:[function(require,module,exports){var math=require("./math");function defaultKeyMap(player){if(player===1){return{0:37,2:38,1:39,3:40,4:90,5:88}}else if(player===2){return{0:83,2:69,1:70,3:68,4:65,5:81}}return null}var keyboardStates={};var keyboardStatesPrev={};var keyMap0=defaultKeyMap(1);var keyMap1=defaultKeyMap(2);var _mousex=0;var _mousey=0;var _mousebtns={};var gamepads=[];var stickSensitivity=.1;function buttonPressed(b){if(typeof b=="object"){return b.pressed}return b==1}exports.btn=function btn(i,player){player=player!==undefined?player:1;var keyCode=0;if(player===1||player===2){if(player===1){keyCode=keyMap0[i]}else if(player===2){keyCode=keyMap1[i]}return!!keyboardStates[keyCode]}else if(player===3){if(gamepads[0]){if(i===4)return buttonPressed(gamepads[0].buttons[0]);else if(i===5)return buttonPressed(gamepads[0].buttons[1]);else if(i===0)return gamepads[0].axes[0]<-stickSensitivity;else if(i===1)return gamepads[0].axes[0]>stickSensitivity;else if(i===2)return gamepads[0].axes[1]<-stickSensitivity;else if(i===3)return gamepads[0].axes[1]>stickSensitivity}}};exports.btnp=function btnp(i,player){player=player!==undefined?player:1;var keyCode=0;if(player===1||player===2){if(player===1){keyCode=keyMap0[i]}else if(player===2){keyCode=keyMap1[i]}return!!keyboardStatesPrev[keyCode]}else if(player===3){return false}};exports.update=function(){var keys=Object.keys(keyboardStates);for(var i=0;i<keys.length;i++){keyboardStatesPrev[keys[i]]=keyboardStates[keys[i]]}updateGamepads()};exports.init=function(canvases){addInputListeners(canvases)};exports.mousex=function mousex(){return _mousex};exports.mousey=function mousey(){return _mousey};exports.mousebtn=function mousebtn(i){return!!_mousebtns[i]};function addInputListeners(canvases){canvasListeners={click:function(evt){if(typeof _click!=="undefined"){updateMouseCoords(evt,canvases);_mousebtns[evt.which]=true;_click();_mousebtns[evt.which]=false}},mousedown:function(evt){_mousebtns[evt.which]=true;updateMouseCoords(evt,canvases)},mouseup:function(evt){_mousebtns[evt.which]=false;updateMouseCoords(evt,canvases)},mouseleave:function(evt){_mousebtns[evt.which]=false;updateMouseCoords(evt,canvases)}};for(var key in canvasListeners){canvases[0].addEventListener(key,canvasListeners[key])}bodyListeners={keydown:function(e){keyboardStates[e.keyCode]=1},keyup:function(e){keyboardStates[e.keyCode]=0},mousemove:function(evt){updateMouseCoords(evt,canvases)}};for(var key in bodyListeners){document.body.addEventListener(key,bodyListeners[key])}}function removeInputListeners(canvases){var key;for(key in canvasListeners){canvases[0].removeEventListener(key,canvasListeners[key])}for(key in bodyListeners){document.body.removeEventListener(key,bodyListeners[key])}}function updateMouseCoords(evt,canvases){if(canvases.indexOf(evt.target)===-1)return;var rect=evt.target.getBoundingClientRect();var parentRect=evt.target.parentNode.getBoundingClientRect();var subx=0;var suby=0;_mousex=(evt.clientX-rect.left-subx)/rect.width;_mousey=(evt.clientY-rect.top-suby)/rect.height}function updateGamepads(){gamepads=navigator.getGamepads?navigator.getGamepads():navigator.webkitGetGamepads?navigator.webkitGetGamepads:[]}exports.mousexNormalized=function(){return _mousex};exports.mouseyNormalized=function(){return _mousey};exports.global={btn:exports.btn,btnp:exports.btnp,mousebtn:exports.mousebtn,click:exports.click}},{"./math":7}],7:[function(require,module,exports){module.exports={sin:Math.sin,cos:Math.cos,flr:Math.floor,ceil:Math.ceil,rnd:function(x){return Math.random()*x},abs:Math.abs,atan2:Math.atan2,max:Math.max,min:Math.min,mix:function(a,b,alpha){return a*alpha+b*(1-alpha)},sgn:Math.sign,sqrt:Math.sqrt,mid:function(x,y,z){var m=z;if(x>=y&&x<=z){m=x}else if(y>=x&&y<=z){m=y}else if(z>=x&&z<=y){m=z}return m},clamp:function(x,min,max){return Math.min(Math.max(x,min),max)}}},{}],8:[function(require,module,exports){var ctx;var writeData=null;var readData=null;var width;var height;var pixelsQueued=0;var tempCanvas=null;var tempCanvasContext=null;exports.init=function(canvas){ctx=canvas.getContext("2d");writeData=ctx.createImageData(canvas.width,canvas.height);width=canvas.width;height=canvas.height;tempCanvas=document.createElement("canvas");tempCanvas.width=width;tempCanvas.height=height;tempCanvasContext=tempCanvas.getContext("2d")};exports.resize=function(canvas){exports.init(canvas)};exports.beforeChange=function(){readData=null;if(pixelsQueued!==0){exports.flush()}};exports.pset=function(x,y,r,g,b){var p=(y*width+x)*4;writeData.data[p+0]=r;writeData.data[p+1]=g;writeData.data[p+2]=b;writeData.data[p+3]=255;pixelsQueued++};exports.pget=function(x,y,out){var p=(y*width+x)*4;if(writeData.data[p+3]===255){out[0]=writeData.data[p+0];out[1]=writeData.data[p+1];out[2]=writeData.data[p+2]}else{if(!readData){readData=ctx.getImageData(0,0,width,height)}out[0]=readData.data[p+0];out[1]=readData.data[p+1];out[2]=readData.data[p+2]}};exports.flush=function(){if(pixelsQueued===0)return;var sourceX=0;var sourceY=0;var destX=0;var destY=0;tempCanvasContext.putImageData(writeData,sourceX,sourceY,destX,destY,width,height);ctx.drawImage(tempCanvas,0,0);pixelsQueued=0;exports.beforeChange();var data=writeData.data;for(var i=0;i<width;i++){for(var j=0;j<height;j++){data[(j*width+i)*4+3]=0}}}},{}],9:[function(require,module,exports){var utils=require("./utils");var DFT=require("dsp.js/dsp").DFT;var minFrequency=0;var maxFrequency=1e3;var context=new createAudioContext;var masterGain=context.createGain();masterGain.gain.value=1;masterGain.connect(context.destination);var oscillatorTypes=["sine","square","triangle","sawtooth"];var allTypes=oscillatorTypes.concat(["square25","white"]);var channels=[];for(var j=0;j<4;j++){var channel={oscillators:{},gains:{},occupiedUntil:-1};channels.push(channel);for(var i=0;i<oscillatorTypes.length;i++){var osc=context.createOscillator();var type=oscillatorTypes[i];osc.type=type;var gain=context.createGain();gain.gain.value=0;gain.connect(masterGain);osc.connect(gain);channel.oscillators[type]=osc;channel.gains[type]=gain;osc.start(context.currentTime)}var gain=context.createGain();gain.gain.value=0;gain.connect(masterGain);var square25=createPulse(gain);channel.oscillators.square25=square25;channel.gains.square25=gain;square25.start(context.currentTime);gain=context.createGain();gain.gain.value=0;gain.connect(masterGain);var whiteNoise=createWhiteNoise(gain);channel.oscillators.white=whiteNoise;channel.gains.white=gain;whiteNoise.start(context.currentTime)}var maxEffects=64;var effects=[];for(var i=0;i<maxEffects;i++){effects.push({types:utils.zeros(maxEffects),frequencies:utils.zeros(maxEffects),volumes:utils.zeros(maxEffects),speed:16})}function createAudioContext(desiredSampleRate){var AudioCtor=window.AudioContext||window.webkitAudioContext;desiredSampleRate=typeof desiredSampleRate==="number"?desiredSampleRate:44100;var context=new AudioCtor;if(/(iPhone|iPad)/i.test(navigator.userAgent)&&context.sampleRate!==desiredSampleRate){var buffer=context.createBuffer(1,1,desiredSampleRate);var dummy=context.createBufferSource();dummy.buffer=buffer;dummy.connect(context.destination);dummy.start(0);dummy.disconnect();context.close();context=new AudioCtor}return context}function play(channel,types,frequencies,volumes,speed,offset){var i,j;var osc=channel.oscillators[allTypes[types[offset+0]]];var gain=channel.gains[allTypes[types[offset+0]]];gain.gain.value=volumes[offset+0]/255;if(osc.frequency){osc.frequency.value=frequencies[offset+0]/255*(maxFrequency-minFrequency)+minFrequency}var endPosition=0;for(i=0;i<volumes.length;i++){if(volumes[i]){endPosition=i+1}}if(endPosition===0)return;var currentTime=context.currentTime;for(i=offset;i<endPosition;i++){var type=allTypes[types[i]];osc=channel.oscillators[type];var startTime=currentTime+i/speed;for(j=0;j<allTypes.length;j++){gain=channel.gains[allTypes[j]];if(allTypes[j]!==type){gain.gain.setValueAtTime(0,startTime)}else{if(osc.frequency){osc.frequency.setValueAtTime(frequencies[i]/255*(maxFrequency-minFrequency)+minFrequency,startTime)}var vol=volumes[i]/255;gain.gain.setValueAtTime(vol,startTime)}}}var len=(endPosition-offset)/speed;var endTime=currentTime+len;for(j=0;j<allTypes.length;j++){gain=channel.gains[allTypes[j]];gain.gain.setValueAtTime(0,endTime)}channel.occupiedUntil=endTime}exports.asset=function(n,speed){effects[n].speed=speed};exports.asget=function(n){return effects[n].speed};exports.avset=function(n,offset,volume){effects[n].volumes[offset]=volume};exports.avget=function(n,offset){return effects[n].volumes[offset]};exports.afset=function(n,offset,frequency){effects[n].frequencies[offset]=frequency};exports.afget=function(n,offset){return effects[n].frequencies[offset]};exports.awset=function(n,offset,waveform){effects[n].types[offset]=waveform};exports.awget=function(n,offset){return effects[n].types[offset]};exports.sfx=function(n,channelIndex,offset){channelIndex=channelIndex!==undefined?channelIndex:-1;offset=offset!==undefined?offset:0;var effect=effects[n];if(!effect){return false}var contextTime=context.currentTime;if(channelIndex===-1){for(var i=0;i<channels.length;i++){var candidateIndex=i;if(channels[candidateIndex].occupiedUntil<contextTime){channelIndex=candidateIndex;break}}}else if(channels[channelIndex].occupiedUntil>=contextTime){return false}if(!channels[channelIndex]){return false}play(channels[channelIndex],effect.types,effect.frequencies,effect.volumes,effect.speed,offset);return true};function createWhiteNoise(destination){var bufferSize=2*context.sampleRate,noiseBuffer=context.createBuffer(1,bufferSize,context.sampleRate),output=noiseBuffer.getChannelData(0);for(var i=0;i<bufferSize;i++){output[i]=Math.random()*2-1}var whiteNoise=context.createBufferSource();whiteNoise.buffer=noiseBuffer;whiteNoise.loop=true;whiteNoise.connect(destination);return whiteNoise}function createPulse(destination){var count=128;var vals2=[];for(var i=0;i<count;i++){var x=i/count;var val=x<.25?-1:1;vals2.push(val)}var a=new DFT(vals2.length);a.forward(vals2);var hornTable=context.createPeriodicWave(new Float32Array(a.real),new Float32Array(a.imag));osc=context.createOscillator();osc.setPeriodicWave(hornTable);osc.connect(destination);return osc}},{"./utils":10,"dsp.js/dsp":1}],10:[function(require,module,exports){exports.disableImageSmoothing=function(ctx){if(ctx.imageSmoothingEnabled!==undefined){ctx.imageSmoothingEnabled=false}else if(ctx.mozImageSmoothingEnabled!==undefined){ctx.mozImageSmoothingEnabled=false}else if(ctx.webkitImageSmoothingEnabled!==undefined){ctx.webkitImageSmoothingEnabled=false}else if(ctx.msImageSmoothingEnabled!==undefined){ctx.msImageSmoothingEnabled=false}};exports.hexToRgb=function hexToRgb(hex){var result=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);return[parseInt(result[1],16),parseInt(result[2],16),parseInt(result[3],16)]};exports.rgbToDec=function(r,g,b){var c=r*(256*256)+g*256+b;return c};exports.decToR=function(c){return Math.floor(c/(256*256))};exports.decToG=function(c){return Math.floor(c/256)%256};exports.decToB=function(c){return c%256};function isSafari(){return navigator.userAgent.indexOf("Safari")!==-1&&navigator.userAgent.indexOf("Chrome")===-1}exports.makeGlobal=function(obj){for(var key in obj){window[key]=obj[key]}};exports.scaleToFit=function scaleToFit(element,containerElement,pixelPerfectMode){var containerWidth=window.innerWidth;var containerHeight=window.innerHeight;if(containerElement){var rect=containerElement.getBoundingClientRect();containerWidth=rect.width;containerHeight=rect.height}var scaleX=containerWidth/element.width;var scaleY=containerHeight/element.height;var scale=Math.min(scaleX,scaleY);var dpr=window.devicePixelRatio||1;if(pixelPerfectMode){scale=Math.floor(scale*dpr)/dpr||1/dpr}var offsetX=(containerWidth-element.width*scale)*.5;var offsetY=(containerHeight-element.height*scale)*.5;if(pixelPerfectMode){offsetX=Math.floor(offsetX*dpr)/dpr;offsetY=Math.floor(offsetY*dpr)/dpr}if(isSafari()){element.style.width=element.width*scale+"px";element.style.height=element.height*scale+"px";element.style.marginLeft=offsetX+"px";element.style.marginTop=offsetY+"px"}else{element.style.transformOrigin="0 0";element.style.transform="translate("+offsetX+"px, "+offsetY+"px) scale("+scale+")"}};exports.fullscreen=function(element){if(document.fullscreenElement)return false;if(element.requestFullscreen){element.requestFullscreen()}else if(element.msRequestFullscreen){element.msRequestFullscreen()}else if(element.mozRequestFullScreen){
element.mozRequestFullScreen()}else if(element.webkitRequestFullscreen){element.webkitRequestFullscreen()}else{return false}return true};exports.zeros=function(n){var a=[];while(n--){a.push(0)}return a};exports.createCanvas=function(w,h){var canvas=document.createElement("canvas");canvas.width=w;canvas.height=h;return canvas}},{}]},{},[5]);